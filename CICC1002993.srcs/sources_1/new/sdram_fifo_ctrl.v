...
// --- 安全的跨时钟域同步与模式切换检测 ---
reg rot_en_d1, rot_en_d2, rot_en_d3;
always @(posedge clk_ref or negedge rst_n) begin
    if (!rst_n) begin
        rot_en_d1 <= 1'b0;
        rot_en_d2 <= 1'b0;
        ...
        rot_en_d1 <= rot_en;
        rot_en_d2 <= rot_en_d1;
        rot_en_d3 <= rot_en_d2; // 多打一拍用于边缘检测
    end
end

wire mode_changed = (rot_en_d2 != rot_en_d3); // 捕捉旋转模式切换的瞬间
...
always @(posedge clk_ref or negedge rst_n) begin
    ...
    else if(wr_load_flag) begin              //写端口复位
        sdram_wr_addr <= rot_en_d2 ? {1'b0, wr_max_addr[22:0] - wr_length + 1'b1} : wr_min_addr;
        sw_bank_en <= 1'b0;
        rw_bank_flag <= 1'b0;
    end
    else if(mode_changed) begin              //强制纠正地址，防分割撕裂！
        // 当切换模式的瞬间，立刻把指针拨回正确的起点，绝对不碰 Bank 翻转标志
        if (rot_en_d2) begin
            // 切换到旋转：回到当前 Bank 末尾
            sdram_wr_addr <= (rw_bank_flag == 1'b0) ? {1'b0, wr_max_addr[22:0] - wr_length + 1'b1} : {1'b1, wr_max_addr[22:0] - wr_length + 1'b1};
        end else begin
            // 切换回正常：回到当前 Bank 开头
            sdram_wr_addr <= (rw_bank_flag == 1'b0) ? {1'b0, wr_min_addr[22:0]} : {1'b1, wr_min_addr[22:0]};
        end
    end
    else if(write_done_flag) begin           //突发写SDRAM结束
        if(sdram_pingpang_en) begin          //SDRAM 读写乒乓使能
            if (!rot_en_d2) begin            //正常正序模式
            ...
            end else begin                   //180度倒序模式
                if(sdram_wr_addr[22:0] > wr_min_addr[22:0])
                    sdram_wr_addr <= sdram_wr_addr - wr_length; 
                else begin                       
                    rw_bank_flag <= ~rw_bank_flag;
                    sw_bank_en <= 1'b1;          
            ...
            if (!rot_en_d2) begin
                ...
                if(sdram_wr_addr > wr_min_addr)
                    sdram_wr_addr <= sdram_wr_addr - wr_length;
                else                                 
                    sdram_wr_addr <= wr_max_addr - wr_length + 1'b1;
            end
        end
    end
    else if(sw_bank_en) begin                //切换BANK
        sw_bank_en <= 1'b0;
        if(rw_bank_flag == 1'b0)             
            sdram_wr_addr <= rot_en_d2 ? {1'b0, wr_max_addr[22:0] - wr_length + 1'b1} : {1'b0, wr_min_addr[22:0]};
        else
            sdram_wr_addr <= rot_en_d2 ? {1'b1, wr_max_addr[22:0] - wr_length + 1'b1} : {1'b1, wr_min_addr[22:0]};
    end
end
//sdram读地址产生模块
always @(posedge clk_ref or negedge rst_n) begin
