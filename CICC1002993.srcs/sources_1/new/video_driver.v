...
// 1280x720@60Hz 时序参数
parameter  H_SYNC   =  11'd40;    // 行同步脉冲宽度
parameter  H_BACK   =  11'd220;   // 行后沿（水平消隐后沿）
parameter  H_DISP   =  11'd1280;  // 行有效像素数
parameter  H_FRONT  =  11'd110;   // 行前沿（水平消隐前沿）
parameter  H_TOTAL  =  11'd1650;  // 行总像素 = H_SYNC + H_BACK + H_DISP + H_FRONT

parameter  V_SYNC   =  11'd5;     // 场同步脉冲宽度
parameter  V_BACK   =  11'd20;    // 场后沿（垂直消隐后沿）
parameter  V_DISP   =  11'd720;   // 场有效行数
parameter  V_FRONT  =  11'd5;     // 场前沿（垂直消隐前沿）
parameter  V_TOTAL  =  11'd750;   // 场总行数 = V_SYNC + V_BACK + V_DISP + V_FRONT
